...
class Appliations(object):
    def __init__(self, home):
        ...
        self.re_action = re.compile('([a-z]+)\s*\(\s*([^)]*)\s*\)',
                                    re.IGNORECASE)
        self.re_icon = re.compile('^\s*(?:\[\s*([a-z0-9-_]+)\s*\])?\s*(.*)',
                                  re.IGNORECASE)
        self.re_disp = re.compile('^\s*NoDisplay\s*=\s*true\s*$',
    ...
    def icons_store(self, iid, path, prefix = ''):
        log.write(log.DEBUG, "Store icons: {0} ({1}, {2})".format(path, prefix,
                                                                  iid))
        if iid:
        ...
        for root, subdirs, files in os.walk(path):
            for item in files:
                ...
                with open(src, 'r') as f:
                    if self.re_disp.search(f.read()) is not None:
                        log.write(log.DEBUG,
                                  "Skipped hidden icon '{0}'".format(src))
                        continue

                if iid:
                    tgt = self.icons_prepare(iid, os.path.relpath(root, path))
                    if tgt is None:
                        continue

                    dst = os.path.join(tgt, item)
                    log.write(log.DEBUG, "Moving icon {0}".format(src))
                    try:
                        shutil.move(src, dst)
                    except Exception as e:
                        Utils.exc(("Failed to move icon '{0}' "
                                   "to '{1}'").format(src, dst), e)
                elif item.startswith(prefix):
                    log.write(log.DEBUG, "Deleting icon {0}".format(src))
                    try:
                        os.remove(src)
                    except Exception as e:
                        Utils.exc("Failed to delete icon '{0}'".format(src), e)
    ...
    def handle_action(self, work, action, args):
        log.write(log.DEBUG, "Adding action '{0}' with args {1}".format(action,
                                                                        args))
        with work.lock:
            if action == 'app':
                app = args[0]
                if app in self.cache:
                    if not self.cache[app].is_installed:
                        log.write(log.DEBUG,
                                  "Marking {0} for installation".format(app))
                        ...
                        log.write(log.DEBUG,
                                "Application {0} already installed".format(app))
                else:
                ...
                log.write(log.WARN,
                    "Unknown action '{0}' in '{1}'".format(action,
                                                           name))
...
class Printers(object):
    ...
    def get_users(self, printer):
        attrs = self.cups.getPrinterAttributes(printer,
    ...
    def refresh(self):
        ...
        log.write(log.DEBUG,
                  "Existing printers: {0}".format(self.printers))
        log.write(log.DEBUG,
                  "Existing printers for user {0}: {1}".format(self.user,
                                                               self.user_printers))
    ...
    def resolve_printer(self, data):
        ...
        if 'path' not in data:
            log.write(log.WARN,
                    "No path specified for printer '{0}'".format(name))
            return False
    ...
    def delete(self, work, name):
        ...
        if self.user in users:
            ...
            if len(users) > 0:
                log.write(log.DEBUG,
                      "Modifying printer {0} (users = {1})".format(name,
                                                                   users))
                with work.lock:
    ...
    def install(self, work, data):
        ...
        if name in self.printers:
            ...
            if self.user in users:
                log.write(log.DEBUG,
                          "Printer {0} already prepared".format(name))
                work.complete(idx)
                return
        else:
            ...
            with work.lock:
                self.cups.addPrinter(name, ppdname = data['ppd'],
                                     info = data['comment'],
        ...
        log.write(log.DEBUG,
                  "Modifying printer {0} (users = {1})".format(name, users))
        with work.lock:
            self.cups.setPrinterUsersAllowed(name, users)
...
class Drives(object):
    ...
    def create_link(self, path, link):
        ...
        if os.path.exists(link):
            log.write(log.WARN,
                      "Entry {0} exists but it's not a link".format(link))
            now = datetime.now()
            ...
            try:
                ...
                Utils.exc("Unable to rename directory {0} to {1}".format(link,
                                                                         tgt),
                          e)
                return False
        try:
    ...
    def mount(self, work, drive):
        ...
        if self.mount_prepare('drive', drive['letter'] + ':'):
            run.command(['mount', '-t', 'cifs', '-o',
                         'vers=1.0,sec=krb5,cruid={0},uid={0},gid={1}'.format(self.uid,self.gid),
    ...
    def bind(self, work, path, target):
        ...
        src = os.path.join('/run/user/', str(self.uid), self.domain, 'drive',
                           path)
        dst = os.path.join('/run/user/', str(self.uid), self.domain, 'bind',
                           target)
...
def user_init():
    ...
    user = ADObj(core.ldap(), filt = '(sAMAccountName={0})'.format(smb.user),
                 extra = ['tokenGroups'])
...
def main():
    ...
    if is_requested('apps') and is_login:
        for location in user_icons:
            apps.icons_store(None, os.path.join(data.pw_dir,
                                                user_icons[location][0]),
                                                user_icons[location][1])
