...
class Appliations(object):
    def __init__(self, home):
        ...
        self.re_disp = re.compile('^\s*NoDisplay\s*=\s*true\s*$', re.IGNORECASE | re.MULTILINE)
    ...
    def mod_desktop_file(self, filename, visible):
        try:
            ...
            config.set("Desktop Entry", "NoDisplay", "{0}".format(not visible).lower())
            with open(filename, 'wb') as desktopfile:
                config.write(desktopfile)
            with open(filename, 'r') as desktopfile:
                filedata = desktopfile.read()
            filedata = filedata.replace(' = ', '=')
            with open(filename, 'w') as desktopfile:
                desktopfile.write(filedata)
        except Exception as e:
            Utils.exc("Unable mod desktop file '{0}'".format(filename), e)
        return None
                ...
                for item in files:
                    ...
                    with open(src, 'r') as f:
                        if self.re_disp.search(f.read()) is not None:
                            log.write(log.DEBUG, "Skipped hidden icon '{0}'".format(src))
                            continue
                    ...
                    if iid:
                        ...
                        log.write(log.DEBUG, "Copying icon {0}".format(src))
                        try:
                            shutil.copy(src, dst)
                            os.chmod(dst, 0755)
                        except Exception as e:
                            Utils.exc("Failed to copy a icon '{0}' to '{1}'".format(src, dst), e)

                        log.write(log.DEBUG, "Hidding icon {0} after copying".format(src))
                        self.mod_desktop_file(src, False)
                    elif item.startswith(prefix):
                        log.write(log.DEBUG, "Hidding custom (_imi_) icon {0}".format(src))
                        try:
                            self.mod_desktop_file(src, False)
                        except Exception as e:
                            Utils.exc("Failed to hide icon '{0}'".format(src), e)
        except Exception as e:
            Utils.exc("Failed in 'icons_store' '{0}'".format(path), e)
    ...
    def icon_install(self, icon):
            ...
            for location in apps_icons:
                if name.startswith(location):
                    avail.append('[{0}]{1}'.format(apps_icons[location], 
                                 os.path.relpath(name[:-8], location)))
        ...
        if len(icon) > 2:
            ...
            for name in icon[2:]:
                ...
                if match is None:
                    log.write(log.ERR, "Invalid icon reference: '{0}'".format(name))
                    continue
                ...
                if key in avail:
                    ...
                    log.write(log.WARN, ("Icon '{0}' not available for application {1}").format(key, app))
        names = []
        for name in tmp:
            match = self.re_icon.match(name)
        ...
        for location in user_icons:
            if (place == 'all') or (place == location):
                paths.append([os.path.join(self.home, user_icons[location][0]), user_icons[location][1]])
        ...
        for path in paths:
            for data in names:
                ...
                log.write(log.DEBUG, "Installing icon '{0}' to {1}".format(name, tgt))
                try:
    ...
    if is_login:
        for item in clean_files:
            try:
                ...
                file_to_remove = os.path.join(data.pw_dir, item)               
