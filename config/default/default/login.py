...
# Location where additional files will be located relative to scripts/NEOS
data_location = 'config/default/default'
...
def run(cmd):
    ...
    try:
        ...
        log(LOG_ERR, "Execution of {0} failed: {1}".format(cmd[0], str(e)))
        return False
    ...
    if p.returncode != 0:
        log(LOG_ERR,
            "Command returned error {0}: {1} ({2})".format(p.returncode,
                                                           cmd[0], err))
        return False
...
def download(servers, share, src, dst):
    ...
    for server in servers:
        ...
        if run(['ping', '-c', '1', '-W', '1', server]) == False:
            log(LOG_WARN,
                "Server {0} seems to be down or it's slow".format(server))
            continue
    ...
    log(LOG_ERR,
        "Unable to download //*/{0}/{1} from any server".format(share, src))
...
def process_block(name):
    if len(sys.argv) == 0:
        return True
    return ('full' in sys.argv) or (name in sys.argv)

cfg = Cfg()

if os.getenv('PAM_SERVICE') not in cfg.get('login', 'services', '').split():
    sys.exit(0)

log_file = None
log_name = cfg.get('login', 'log')
if log_name:
    try:
...
if username is None:
    log(LOG_INFO, "No kerberos session found")
    sys.exit(0)
...
log(LOG_DEBUG, "User: {0}".format(username))
log(LOG_DEBUG, "Domain: {0}".format(domain))
log(LOG_DEBUG, "Home: {0}".format(user['home']))
log(LOG_DEBUG, "UID: {0}".format(user['uid']))
log(LOG_DEBUG, "GID: {0}".format(user['gid']))

wsclass = cfg.get('workstation', 'class', 'default')
wsenv = cfg.get('workstation', 'environment', 'default')

log(LOG_DEBUG, "Class: {0}".format(wsclass))
log(LOG_DEBUG, "Environment: {0}".format(wsenv))

base_dir = os.path.join(domain, 'scripts/NEOS', data_location)
...
def icons_mkdir(*path):
    try:
        path = os.path.join('/opt/dl/applications', path)
        if not os.path.exists(path):
            os.mkdir(path)
        return path
    except Exception as e:
        log(LOG_ERROR, "Unable to create directory '{0}': {1}".format(path,
                                                                      str(e)))
    return None

def icons_prepare(iid, path = '.'):
    if icons_mkdir(iid):
        return icons_mkdir(iid, path)
    return None

def icons_store(iid, path):
    if iid:
        if not icons_prepare(iid):
            return False
    ...
    for root, subdirs, files in os.walk(path):
        for item in files:
            ...
            with open(src, 'r') as f:
                if not re_disp.search(f.read()):
                    log(LOG_DEBUG, "Skipped hidden icon '{0}'".format(src))
                    continue

            if iid:
                tgt = icons_prepare(iid, os.path.relpath(root, path))
                if not tgt:
                    continue

                dst = os.path.join(tgt, item)
                log(LOG_DEBUG, "Moving icon {0}".format(src))
                shutil.move(src, dst)
            elif item[:5] == '_imi_':
                log(LOG_DEBUG, "Deleting icon {0}".format(src))
                os.remove(src)

re_disp = re.compile('^\s*NoDisplay\s*=\s*true\s*$',
                     re.IGNORECASE | re.MULTILINE)
if icons_mkdir():
    icons_store('common', '/usr/share/applications')
    icons_store('mate', '/usr/share/mate/applications')
    icons_store(None, os.path.join(home, 'Desktop'))
    icons_store(None, os.path.join(home, '.local/share/applications'))
